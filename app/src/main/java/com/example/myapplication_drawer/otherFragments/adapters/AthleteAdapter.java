...
public class AthleteAdapter  extends RecyclerView.Adapter<AthleteAdapter.ViewHolder>{
    private ArrayList<Athlete> localDataSet = new ArrayList<Athlete>();
     ...
     * by RecyclerView.
     */
    public AthleteAdapter(ArrayList<Athlete> dataSet) {
    ...
    @NotNull
    @Override
    public AthleteAdapter.ViewHolder onCreateViewHolder(ViewGroup viewGroup, int viewType) {
        ...
        return new AthleteAdapter.ViewHolder(textview);
    }

    // Replace the contents of a view (invoked by the layout manager)
    @Override
    public void onBindViewHolder(AthleteAdapter.ViewHolder viewHolder, final int position) {

        // Get element from your dataset at this position and replace the
        // contents of the view with that element
        viewHolder.textView.setText("Κωδικός Αθλητή: " + localDataSet.get(position).getId() + "\n" + "Όνομα: " + localDataSet.get(position).getName() + "\n" + "Επώνυμο: " +  localDataSet.get(position).getSurname() + "\n" + "Πόλη: " +  localDataSet.get(position).getTown() + "\n" + "Χώρα: " +  localDataSet.get(position).getCountry() + "\n" + "Έτος Γέννησης: " +  localDataSet.get(position).getDate() + "\n" + "Κωδικός Αθλήματος: " +  localDataSet.get(position).getSport_id());
    }
